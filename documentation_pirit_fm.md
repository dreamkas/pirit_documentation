...
<<<<<< Punix
# Инструкция по программированию punix
<<<<<< common
...
Номер запроса (DEC) | Наименование Запроса | Формат возвращаемых данных | Комментарии
--- | --- | --- | ---
<<<<<< Pirit2f 1.05 Pirit2f 1.1 Punix
...
<<<<<< FM16 1.05 FM16 1.1 FM16 1.2 Punix
...
| 80           | Обновите прошивку для работы с данными агента и поставщика   |
| 81           | Неверный параметр "Код товарной номенклатуры"   |
| 82           | Превышено кол-во позиций с Кодом товарной номенклатуры   |
| 83           | Параметр "Название товара" не соответствует параметру "Признак предмета расчета"   |
| 84           | Обновите прошивку ККТ, чтобы передать дополнительный реквизит чека (бсо)   |
| 85           | Неверный параметр "ИНН покупателя"   |
| 86           | Неверный параметр "Покупатель"   |
| 87           | Обновите прошивку ККТ, чтобы передать данные о покупателе   |
| 88           | Обновите прошивку ККТ для работы с дополнительными реквизитами ФФД 1.1   |
...
| 92           | Услуга ФФД 1.05 не активирован   |
...
<<<<<< FM16 1.2
 * (Строка) Адрес пользователя (тэг 1009). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
 * (Строка) Место расчетов (тэг 1187). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
<<<<<< common
...
## Входные параметры
 * (Строка[1..32]) Код товарной номенклатуры (Тег 1162)
 * (Строка[1..64]) Дополнительный реквизит предмета расчёта (Тег 1191)
 * (Строка[1..16]) Единица измерения предмета расчёта (Тег 1197)
 * (Целое число) Признак агента по предмету расчёта (Тег 1057)
 * (Строка)[0..12] ИНН поставщика (Тег 1226)
<<<<<< Pirit2f 1.1
 * (Строка)[0..60] Телефон(ы) поставщика (Тег 1171)
<<<<<< FM16 1.05 FM16 1.1 FM16 1.2 Pirit2f 1.05 Punix
 * (Строка)[0..19] Телефон поставщика (Тег 1171)
<<<<<< common
...
<<<<<< Pirit2f 1.1
 * (Строка)[0..60] Телефон(ы) оператора перевода (для банк.пл.агента/субагента, иначе пустой) (Тег 1075)
<<<<<< FM16 1.05 FM16 1.1 FM16 1.2 Pirit2f 1.05 Punix
 * (Строка)[0..19] Телефон оператора перевода (для банк.пл.агента/субагента, иначе пустой) (Тег 1075)
<<<<<< common
...
<<<<<< FM16 1.05 FM16 1.1 FM16 1.2 Pirit2f 1.05 Punix
 * (Строка)[0..19] Телефон платежного агента (для пл.агента/субагента, иначе пустой) (Тег 1073)
 * (Строка)[0..19] Телефон оператора по приему платежей (для пл.агента/субагента, иначе пустой) (Тег 1074)
<<<<<< FM16 1.2
 * (Число)[0..3] Идентификатор ФОИВ (тэг 1262). Это поле включается в случае, если это предусмотрено законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Дата) Дата документа основания (тэг 1263). Допускается дата после 1999 года. Это поле включается в случае, если это предусмотрено законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Строка)[0..32] Номер документа основания (тэг 1264). Это поле включается в случае, если это предусмотрено законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Строка)[0..256] Значение отраслевого реквизита (тэг 1265). Это поле включается в случае, если это предусмотрено законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
<<<<<< common


## Ответные параметры
 * (Строка)[0..8] Результат проверки поля "Код товарной номенклатуры" (Реализован, начиная с версии 665.3.0, при некорректном поле будет возвращено значение "Unrecogn")

## Дополнительная информация
**Код товарной номенклатуры**

В поле «Код товарной номенклатуры» могут быть переданы управляющие символы (0x00..0x1F) в виде: $xy, где x и y – шестнадцатиричные цифры, например $15 соответствует значению 0x15. Количество предметов расчёта, для которого задан код товарной номенклатуры Datamatrix, должно равняться единице.
При регистрации в режиме ФФД 1.2 поле «Код товарной номенклатуры» не используется.
...
В режиме ФФД 1.2 поле не используется, необходимо применять команды 0x79.
...
Признак агента может использоваться только при указании признаков агента при регистрации ККТ.

<<<<<< FM16 1.2
# Внесение дополнительных реквизитов (0x25)
## Описание
Команда используется при добавлении дополнительных реквизитов в отчеты и вызывается ПЕРЕД соответствующей командой.
...
Могут быть бинарными, передаваемые через $, как при передаче ктн в команде 42
...
Применяемая система налогообложения должна быть указана при регистрации ККТ.
...
<<<<<< FM16 1.2
При ФФД 1.2 фискальный признак также возвращается при завершении чеков коррекции на приход, возврат прихода, расход и возврат расхода.
<<<<<< common
В пакетном режиме формирования документа, команда “Завершить документ” выключает пакетный режим.

## Входные параметры
 * (Целое число) Флаг отрезки
 * (Строка) Адрес покупателя (Тег 1008)
 * (Число) Разные флаги
<<<<<< Pirit2f 1.05 Pirit2f 1.1 Punix FM16 1.2
 ...
 * (Строка) Зарезервировано
<<<<<< common
 * (Строка) Название дополнительного реквизита пользователя (Тег 1085)
...
<<<<<< FM16 1.05 FM16 1.1 FM16 1.2 Pirit2f 1.1 Punix
 * (Строка)[0..128] Покупатель (Тег 1227)
<<<<<< common
 * (Строка)[0..12] ИНН покупателя (Тег 1228)
<<<<<< FM16 1.2
 * (Дата в формате ddmmyyyy) Дата рождения покупателя (тэг 1243). Это поле включается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Число)[0..3] Гражданство (тэг 1244). Это поле включается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Число)[0..2] Код вида документа, удостоверяющего личность (тэг 1245). Это поле включается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Строка)[0..64] Данные документа, удостоверяющего личность (Тег 1246). Это поле включается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Строка)[0..256] Адрес покупателя (клиента), географический адрес, не email (Тег 1254). Это поле включается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
<<<<<< common
...
При регистрации ККТ в режиме ФФД 1.2 и работе с маркированным товаром допускается передача данных в формате n/m,
где n - количество проданного товара (тэг 1293), а m - количество товара в упаковке/партии (тэг 1294), подлежащей обязательной маркировке.
При такой передаче m и n - целые положительные числа и n < m.
<<<<<< common
...
<<<<<< FM16 1.2 Pirit2f 1.05 Pirit2f 1.1 Punix
...
| 1273 **  | Дата операции (ддммгг)  |
* - Тэги отраслевого реквизита (включаются в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2).
** - Тэги операционного реквизита (определяются ФНС РФ, включаются только при регистрации ККТ в режиме ФФД 1.2).
<<<<<< common
...
<<<<<< FM16 1.2
**Наименование основания коррекции**

Поле оставлено для совместимости, не используется в соответствии с актуальными ФФД
...
Дополнительный режим работы. Представляет собой битовую маску.

| Номер бита | Пояснение                                |
| ---------- | ---------------------------------------- |
| 0          | Устройство для печати находится вне корпуса ККТ |
<<<<<< FM16 1.2
| 1          | Применение маркированных товаров         |
| 2          | Применение ломбардами                    |
| 3          | Применение в страховой деятельности      |

Тип ФФД представляет собой число. Если он равен 2, ККТ будет работать в режиме ФФД 1.05, если равен 4 - в режиме ФФД 1.2.
 ...
 * (Имя кассира) Имя кассира
<<<<<< Punix
 * (Строка) Адрес пользователя, строка 1 (тэг 1009). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
 * (Строка) Адрес пользователя, строка 2 (тэг 1009). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
 * (Строка) Место расчетов (тэг 1187). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
<<<<<< FM16 1.2
 * (Строка) Адрес пользователя (тэг 1009). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
 * (Строка) Место расчетов (тэг 1187). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
...
# Запрос статуса по работе с кодами маркировки (0x79 запрос 0)
## Описание
С помощью этой команды можно получить состояние КМ.
 ...
 * (Целое число) Предупреждение о заполнении области хранения уведомлений о реализации маркированного товара
...
# Передача КМ в ФН для проверки (0x79 запрос 1)
## Описание
 ...
 * (Целое число) Планируемый статус товара(тэг 2003)
 * (Целое число) Количество товара (тэг 1023)
 * (Целое число) Мера количества (тэг 2108)
 * (Целое число) Режим работы

## Ответные параметры
 * (Целое число) Номер запроса = 1
 * (Целое число) Результат проверки КМ в ФН (тэг 2106)
 * (Целое число) Причина того, что КМ не проверен в ФН
 * (Целое число) Результаты обработки запроса (тэг 2005)
 * (Целое число) Код обработки запроса (тэг 2105)
 * (Целое число) Сведения о статусе товара (тэг 2109)

## Дополнительная информация

В онлайн-режиме ККТ передаст запрос на проверку КМ  на сервер ОИСМ и 3 сек ждет ответа от сервера.
При невозможности проверить код в ИСМ, тэги 2005, 2105 и 2109 возвращаются пустыми

**Код маркировки**

В КМ могут быть переданы управляющие символы (0x00..0x1F) в виде: $xy, где x и y – шестнадцатиричные цифры, например $15 соответствует значению 0x15

**Количество товара**

Если мера количества (тэг 2108) = 0 (штучный товар) допускается передача данных в формате n/m, где n - количество проданного товара (тэг 1293), а m - количество товара в упаковке/партии (тэг 1294), при такой передаче m и n - целые положительные числа и n < m)
...
## Описание
Эта команда сохраняет КМ в ФН для возможности включения КМ в состав позиции чека.
...
# Очистка сохраненных КМ в ФН (0x79 запрос 3)
## Описание
С помощью этой команды можно очистить сохраненные КМ в ФН.
При закрытии документа ФН очистит сохраненные КМ автоматически.
...
# Запрос состояния передачи уведомлений о КМ (0x79 запрос 6)
...
# Запрос длины уведомления о КМ (0x79 запрос 7)
...
# Запрос блока уведомления о КМ (0x79 запрос 8)

Команда используется для получения данных уведомления из ФН.
...
# Окончание чтения уведомления о КМ (0x79 запрос 9)
...
# Передача квитанции на уведомление о КМ (0x79 запрос 10)

## Входные параметры
 * (Целое число) Номер запроса = 10
 * (Целое число) Размер квитанции
 * (Массив HEX) Сама квитанция

## Ответные параметры
 * (Целое число) Номер запроса = 10
 * (Целое число) Результаты обработки уведомления (тэг 2006)
 * (Целое число) Коды обработки уведомления (тэг 2111)
...
В случае получения ошибки 0x60, то в этом поле возвращается “Причина ошибки при обработке квитанции”
...
# Запрос выгрузки уведомлений о КМ в автономном режиме (0x79 запрос 11)
...
# Запрос длины очередного уведомления о КМ в автономном режиме (0x79 запрос 12)
...
# Запрос очередного уведомления о КМ в автономном режиме (0x79 запрос 13)
...
# Подтверждение выгрузки очередного уведомления о КМ в автономном режиме (0x79 запрос 14)
...
# Передача КМ для включения в документ (0x79 запрос 15)

## Входные параметры
 * (Целое число) Номер запроса = 15
 * (Строка)[0..128] Код маркировки
 * (Целое число) Присвоенный статус товара (тэг 2110)
 * (Целое число) Режим обработки кода маркировки (тэг 2102) = 0
 * (Целое число) Результат проведенной проверки КМ (тэг 2106)
 * (Целое число) Мера количества (тэг 2108)
...
Параметры 'Код маркировки' и 'Мера количества' должны совпадать с переданными в команде 0x79/1
 ...
 * (Число[1..65534]) - Порт ОИСМ-сервера (если задано занчение 0, будет использован порт ОФД-сервера)
<<<<<< common
