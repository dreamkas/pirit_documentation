...
В поле «Код товарной номенклатуры» могут быть переданы шестнадцатеричные данные, каждый такой байт имеет вид: $xy, где x и y – шестнадцатиричные цифры, например $C5 соответствует значению 0xC5. Количество предмет расчёта, для которого задан код товарной номенклатуры, должно равняться единице. Поле передается в ФН без изменений. Структура его определяется ФФД.
<<<<<< FM16 1.05 FM16 1.1
...
<<<<<< Pirit2f 1.05
 * (Строка)[0..128] Покупатель (для прошивки 565.0.12 при печати на нулевом дизайне максимальная длина - 82 символа)
 ...
 * (Строка[0...32]) Номер таможенной декларации
<<<<<< FM16 1.05 FM16 1.1 Punix
