...
## Описание
Команда предназначена для загрузки графического логотипа торгового предприятия во внутреннюю память ККТ с последующей печатью перед каждым документом.
...
<<<<<< FM16 1.2
 * (Число)[0..3] Идентификатор ФОИВ (тэг 1262). Значение определяется ФНС РФ. Параметр используется только при регистрации ККТ в режиме ФФД 1.2.
 * (Дата) Дата документа основания (тэг 1263). Допускается дата после 1999 года. Должен содержать сведения об НПА отраслевого регулирования. Параметр используется только при регистрации ККТ в режиме ФФД 1.2.
 * (Строка)[0..32] Номер документа основания (тэг 1264). Должен содержать сведения об НПА отраслевого регулирования. Параметр используется только  при регистрации ККТ в режиме ФФД 1.2.
 * (Строка)[0..256] Значение отраслевого реквизита (тэг 1265). Значение определяется отраслевым НПА. Параметр используется только  при регистрации ККТ в режиме ФФД 1.2.
<<<<<< common
...
**Код товара (тэг 1162 для ФФД 1.05)**

Реквизит включается в состав фискального документа при расчетах за товары, маркированные средствами идентификации.
В поле могут быть переданы управляющие символы (0x00..0x1F) в виде: $xy, где x и y – шестнадцатиричные цифры, например $15 соответствует значению 0x15. 
Количество предметов расчёта, для которого задан код товара Datamatrix, должно равняться единице.
В случае некорректного значения поля документ будет сформирован, но код товара будет передан, как нераспознанный.
При регистрации в режиме ФФД 1.2 тэг 1162 не используется.
...
При регистрации ККТ в режиме ФФД 1.2 поле не используется, для формирования фискальных документов при расчетах за товары, маркированные средствами идентификации необходимо применять команды 0x79.
 ...
 * (Число 0..5) Система налогообложения (Тэг 1055)
<<<<<< Punix
 * (Строка) Адрес пользователя, строка 1 (Тэг 1009)
 * (Строка) Адрес пользователя, строка 2 (Тэг 1009)
...
<<<<<< FM16 1.2
 * (Дата в формате ddmmyyyy) Дата рождения покупателя (тэг 1243). Параметр передается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Число)[0..3] Гражданство (тэг 1244). Параметр передается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
 * (Число)[0..2] Код вида документа, удостоверяющего личность (тэг 1245). Параметр передается в случаях, установленных законодательством РФ и только при регистрации ККТ в режиме ФФД 1.2.
...
Используя параметры печати отчета о закрытии смены в “Таблица настроек ККТ”, можно настроить в Отчете о закрытии смены” печать информации (сумма и количество) по отложенным за смену чекам. При этом, если команда "Отложить чек" выполняется без параметра (пустая строка), то итоговая сумма по отложенным чекам за смену фиксируется в ККТ и отображается в “Отчете о закрытии смены”, если с параметром – не фиксируются.
