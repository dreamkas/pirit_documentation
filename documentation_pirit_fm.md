...
КМ должен быть передан без предварительного разбора, в “сыром” виде. Управляющие символы (0x00..0x1F) содержащиеся в КМ могут быть переданы в виде: $xy, где x и y – шестнадцатиричные цифры, например $15 соответствует значению 0x15
...
## Описание
Команда предназначена для  удаления сохраненных КМ в ФН, подлежащих включению в “Уведомление о реализации маркированного товара”.
При закрытии фискального документа (кассового чека), ФН удалит сохраненные КМ автоматически.
...
<<<<<< FM16 1.2
# Установка параметров WiFi (0x85)

## Входные параметры
 * (Целое число) Тип настройки
 * (Строка)[0..32] Имя сети
 * (Строка)[0..64] Пароль в сети
...
| Значение  | Пояснение                                             |
| --------- | ----------------------------------------------------- |
| 0         | Сеть для подключения                                  |
| 1         | Параметры точки доступа wifi модуля                   |

# Чтение параметров WiFi (0x86)

## Входные параметры
 * (Целое число) Тип запроса

## Ответные параметры
 * (Строка)[0..32] Параметр 1
 * (Строка)[0..15] Параметр 2
...
В случае значения 0 в параметре 1 возвращается имя сети, в параметре 2 - IP-адрес.
В случае значения 1 в параметре 1 возвращается имя точки доступа, в параметре 2 - IP-адрес.
В случае значения 2 в параметре 1 MAC-адрес Wifi модуля.
