...
При такой передаче m и n - целые положительные числа и n < m. При формировании чека с «дробным количество маркированного товара» (тег 1291) ККТ автоматически
сформирует значение реквизита «количество предмета расчета» (тег 1023) равное «1» для передачи в ОФД и формирования печатной формы чека в соответствии с
утвержденными ФФД.
...
# Получить запрос на обновление ключей проверки (0x79 запрос 50)

## Входные параметры
 * (Целое число) Номер запроса = 50

## Ответные параметры
 * (Целое число) Номер запроса = 50
 * (Массив HEX) Данные блока запроса

# Передать ответ на запрос на обновление ключей проверки (0x79 запрос 51)

## Входные параметры
 * (Целое число) Номер запроса = 51
 * (Целое число) Длина ответа
 * (Массив HEX) Данные ответа

## Ответные параметры
 * (Целое число) Номер запроса = 51

## Дополнительная информация
При возвращении ошибки 0x70 команду 0x79 запрос 50 следует повторить.

# Запрос состояния ключей в ФН (0x79 запрос 52)

Команда может не поддерживаеться ФН.

## Входные параметры
 * (Целое число) Номер запроса = 52
 * (Целое число) Номер подзапроса (0..2)
...
| Значение  | Пояснение                                             |
| --------- | ----------------------------------------------------- |
| 0         | Проверить необходимость обновления ключей (если возвращается 1 - нужно обновить ключи) |
| 1         | Запросить URL сервера обновления ключей |
| 2         | Вернуть список идентификаторов ключей ФН |
