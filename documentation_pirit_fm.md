 ...
 * (Дата) Дата документа основания (тег 1263). Допускается дата после 1999 года. Должен содержать сведения об НПА отраслевого регулирования. Параметр используется только при регистрации ККТ в режиме ФФД 1.2.
...
**Код товара (тег 1162 для ФФД 1.05)**
...
При регистрации в режиме ФФД 1.2 тег 1162 не используется.
...
По требованиям законодательства цена товара всегда должна передаваться с учётом всех скидок и наценок.
...
После повторной команды «Подытог», изменяется статус документа(на состояние оплаты). Далее мы можем распечатать дополнительные реквизиты, прервать оформление чека командами «Отложить чек» и «Аннулировать чек», либо продолжить оформление документа, выполнив команду «Оплата» и команду «Завершить документ».
...
Код типа платежа должен соответствовать одному из запрограммированных кодов типа платежа “Таблице настроек ККТ”.
...
## Описание
Команда предназначена для печати и формирования в электронной форме дополнительных реквизитов чека, при открытом чеке на продажу, возврат, внесения или изъятия.

Код реквизита должен соответствовать одному из запрограммированных наименований реквизита в “Таблице настроек ККТ”(1..5). Если значение реквизита обеспечивает однозначное его толкование, его можно распечатать без предварительно запрограммированного наименования, с кодом реквизита равным нулю.
...
## Описание
Команда регистрирует сумму по указанному отделу для одного из типов операции продажа или возврат. Эта команда может быть вызвана после любой команды при открытом документе.
...
В случае передачи во входных параметрах команды номера ФД данные итога чека, печатаемые в подвале чека, будут браться из ФН (документ в ФН должен быть сформирован в текущей смене).
...
# Формирование и печать чека коррекции (0x58)
## Описание
 ...
 * (Имя кассира) Имя кассира
<<<<<< Punix
 * (Строка) Адрес пользователя, строка 1 (тег 1009). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
 * (Строка) Адрес пользователя, строка 2 (тег 1009). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
 * (Строка) Место расчетов (тег 1187). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
<<<<<< FM16 1.2
 * (Строка) Адрес пользователя (тег 1009). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
 * (Строка) Место расчетов (тег 1187). Это поле используется, если указанные реквизиты отличны от реквизитов, переданных при формировании отчета о регистрации ККТ.
